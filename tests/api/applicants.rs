...
use generate_coding_challenge_server::{
    domain::{algo_question::one_edit_away, Nuid},
...
async fn applicants_returns_a_200_for_valid_nuids_that_exist() {
    ...
    let nuid1 = Nuid::parse(nuid1.into()).unwrap();
    assert!(actual
        .applicants_found
        .iter()
    ...
    let nuid2 = Nuid::parse(nuid2.into()).unwrap();
    assert!(actual
        .applicants_found
        .iter()
    ...
    let nuid3 = Nuid::parse(nuid3.into()).unwrap();
    assert!(actual
...
async fn applicants_returns_a_404_for_a_mix_of_valid_nuids_that_do_and_dont_exist() {
    ...
    let nuid4 = Nuid::parse(nuid4.into()).unwrap();
    assert!(actual
